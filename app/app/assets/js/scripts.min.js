...
$(document).ready(function () {
  ...
  $('body').on('mousedown touchend', function () {
    $('.socialPopup').remove();
  });
});
...
var highlighter = function highlighter(target) {
  var makePopup = function makePopup(url, title, text, style, className) {
    ...
    $('body').find('.socialPopup').on('mousedown touchend', function (e) {
      console.log('hey');
      e.preventDefault();
      e.stopPropagation();
    });
  ...
  var isInverse = function isInverse(selection) {
    ...
    var offsetBottom = rects[rects.length - 1].bottom + 60;
  ...
  var getTop = function getTop(selection) {
    if (window.IS_TOUCH_SCREEN) {
      ...
      var offsetBottom = _rects[_rects.length - 1].bottom + 60;
  ...
  $(target).on('selectionchange', function (e) {
    ...
    var $focus = $(anchorNode.parentNode);
    var $first = $anchor.index() < $focus.index() ? $anchor : $focus;

    if ($anchor.data('highlight') || $focus.data('highlight')) {
      if ($first.data('highlight') === 'next') {
        var $next = $first.next();
        var timestamp = $next.data('start') || $next.attr('timestamp');
        url = url + '#' + timestamp;
      } else if ($first.data('highlight') === 'transcript' || $focus.data('highlight' === 'transcript')) {
        var _timestamp = $first.data('start') || $first.data('timestamp') || $focus.data('start') || $focus.data('timestamp');
        url = url + '#' + _timestamp;
      }
    }
...
},{"./getUrlWithNoHash":64}],68:[function(require,module,exports){
'use strict';
